      ...
      <div class="table standard-padding px-3">
        <section class="table__body">
          <table class="table table-md-responsive" id="dateTable" >
            ...
            $sql="select Month(date) as month,SUM(total) as total_data from sell_data Group BY month";
            $result=mysqli_query($connection,$sql);
            if($result){
              while($row=mysqli_fetch_assoc($result)){
                ...
                <td>' .$month. '</td> 
                <td>' .$total_data. '</td>    
                <td><a href="detail_month_list.php?month='.$month.'" target="_blank">Detail</a></td>
