...
function saveChangesForRow(row) {
    ...
    for (let j = 0; j < row.cells.length; j++) {
        ...
        xhr.open('POST', 'index2_update.php', true);
  ...
  function deleteRow(button) {
    ...
    const tra_id = row.cells[8].textContent; // Assuming the Product ID is in the first column (index 0)
  
    // Send the tra_id to the backend PHP script using AJAX for deletion
    const xhr = new XMLHttpRequest();
    xhr.open('POST', 'index2_delete.php', true);
