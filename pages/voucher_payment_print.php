  ...
  <div class="voucher-container">
      ...
      <tr>
        <td colspan="2" class="voucher-header">
            ...
            <?php
              ...
              if (isset($_GET['voucher']) && isset($_GET['customer']) && isset($_GET['payable']) && isset($_GET['pay'])) {
                $voucher = $_GET['voucher'];
                $customer = urldecode($_GET['customer']);
                $payable = urldecode($_GET['payable']);
                $pay = urldecode($_GET['pay']);
    ...
    <?php
      ...
      if ($connection->connect_error) {
        ...
        $sql = "SELECT * FROM sell_data WHERE voucher='$voucher'";
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $sql = "SELECT SUM(total) AS total_sum, SUM(discount) As total_discount from sell_data where voucher='$voucher'"; // Use an alias "total_sum" for the sum(total) column
            $result = $connection->query($sql);
            ...
            if ($result->num_rows > 0) {
                ...
                echo "<tr class='user_table_td'>                 
                    <td colspan=4 style='text-align:center;'>ပေးချေပြီးငွေ</td>
                    <td><b>" . $pay . "</td></b>
                    </tr>";

                echo "<tr class='user_table_td'>                 
                    <td colspan=5 style='text-align:center;'><b>ငွေချေပြီး ပေးရန်ကျန်ငွေ-".$payable."</td></b>
                </tr>";
      ...
      <tr>
        ...
        $today = date("Y-m-d");
          ...
          <td class="voucher-footer" colspan="2">Date: '.$today.'<br>
            <label>ငွေပေးချေမှုအတွက် အထူးကျေးဇူးတင်ရှိပါသည်</label>
        </td>
          ';
        ?>
